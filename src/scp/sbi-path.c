...
int scp_sbi_open(void)
{
    ...
    ogs_list_for_each(&ogs_sbi_self()->nf_instance_list, nf_instance) {
        ...
        ogs_sbi_nf_instance_build_default(nf_instance, scp_self()->nf_type);
#if 0
        ...
        service = ogs_sbi_nf_service_build_default(nf_instance,
                (char*)OGS_SBI_SERVICE_NAME_NSCP_MANAGEMENT);
        ogs_assert(service);
...
bool scp_nnrf_nfm_send_nf_register(ogs_sbi_nf_instance_t *nf_instance)
{
    ...
    request = scp_nnrf_nfm_build_register(nf_instance);
